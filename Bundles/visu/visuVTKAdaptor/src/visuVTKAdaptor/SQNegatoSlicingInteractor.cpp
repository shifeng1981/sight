...
static ::fwServices::QtQmlRegistar<SQNegatoSlicingInteractor>   registar("::visuVTKAdaptor::SNegatoSlicingInteractor");

class NegatoQSlicingCallback : public vtkCommand
{
public:
    //------------------------------------------------------------------------------

    static NegatoQSlicingCallback* New()
    {
        return new NegatoQSlicingCallback();
    }

    NegatoQSlicingCallback() :
        m_picker(nullptr),
...
const ::fwCom::Signals::SignalKeyType SQNegatoSlicingInteractor::s_SLICING_STARTED_SIG = "slicingStarted";
const ::fwCom::Signals::SignalKeyType SQNegatoSlicingInteractor::s_SLICING_STOPPED_SIG = "slicingStopped";

static const ::fwServices::IService::KeyType s_IMAGE_INOUT = "image";

//-----------------------------------------------------------------------------

SQNegatoSlicingInteractor::SQNegatoSlicingInteractor() noexcept :
    ...
    m_priority(.6f)
{
    ...
    newSlot(s_UPDATE_SLICE_INDEX_SLOT, &SQNegatoSlicingInteractor::updateSliceIndex, this);
    newSlot(s_UPDATE_SLICE_TYPE_SLOT, &SQNegatoSlicingInteractor::updateSliceType, this);
}
...
void SQNegatoSlicingInteractor::configuring()
{
...
void SQNegatoSlicingInteractor::starting()
{
    this->initialize();

    NegatoQSlicingCallback* observer = NegatoQSlicingCallback::New();
...
void SQNegatoSlicingInteractor::updating()
{
    ::fwData::Image::sptr image = this->getInOut< ::fwData::Image >(s_IMAGE_INOUT);
    SLM_ASSERT("Missing image", image);
    this->updateImageInfos(image);
...
void SQNegatoSlicingInteractor::stopping()
{
...
void SQNegatoSlicingInteractor::updateSliceType(int from, int to)
{
...
void SQNegatoSlicingInteractor::startSlicing( double pickedPoint[3] )
{
...
void SQNegatoSlicingInteractor::stopSlicing()
{
    ::fwData::Image::sptr image = this->getInOut< ::fwData::Image >(s_IMAGE_INOUT);
    SLM_ASSERT("Missing image", image);
...
void SQNegatoSlicingInteractor::updateSlicing( double pickedPoint[3] )
{
    ::fwData::Image::sptr image = this->getInOut< ::fwData::Image >(s_IMAGE_INOUT);
    SLM_ASSERT("Missing image", image);
...
void SQNegatoSlicingInteractor::pushSlice( int factor, Orientation axis)
{
    ::fwData::Integer::sptr sliceIndex[3];
    this->getSliceIndex(sliceIndex);
