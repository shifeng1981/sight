 ...
 * @code{.xml}
          <service uid="..." type="::uiIO::editor::SQIOSelector">
...
public:
     ...
     *
     * By default, the SQIOSelector::m_mode is defined as READER_MODE, and SQIOSelector::m_servicesAreExcluded as true.
     */
    UIIO_API SQIOSelector();

    /// Destructor. Do nothing.
    UIIO_API virtual ~SQIOSelector() noexcept;

    /**
     * @brief This method allows to configure the service in reader or writer mode (set SQIOSelector::m_mode).
     *
     *@param[in] _mode the value can be SQIOSelector::READER_MODE or SQIOSelector::WRITER_MODE.
     */
...
protected:
     ...
     * Thanks to this method, SQIOSelector::m_selectedServices value is up to date (cleared before reconfiguring).
     **/
...
Q_SIGNALS:
    void    jobCreated(void);
    /**
     * @brief: this signal is send when read is done
     * If reader fail and do not throw an exception, this signal will be emitted.
     */
    void    done(void);
    /**
     * @brief failure
     * If the reader fail and throw a error, this signal will be emitted.
...
private:
     ...
     *
     * @see SQIOSelector::m_servicesAreExcluded.
     */
