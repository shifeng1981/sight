...
void AddDistance::updating()
{
    ...
    if (image.get()->getSizeInBytes() == 0)
    {
        SLM_WARN("Image content is not found.");
        return;
    }
    ...
    std::transform( pt2->getCoord().begin(), pt2->getCoord().end(), image->getSpacing().begin(),
                    pt2->getCoord().begin(), std::multiplies<double>());
    std::transform( pt2->getCoord().begin(), pt2->getCoord().end(), image->getOrigin().begin(),
                    pt2->getCoord().begin(), std::plus<double>());
